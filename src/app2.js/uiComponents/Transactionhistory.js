...
const newDate = date.toLocaleString('default', { month: 'numeric',day:'numeric',year:'numeric',yearYY:true,hour:'numeric',hour12:true,minute:'numeric' });
return newDate;
  }
              ...
              <div className={`mx-3 d-flex ${css.list_body}`}>
                ...
                <div className="d-flex flex-column font-8">
                  ...
                  <div className={`${css.games_section_headline}`}>
                    Status:{(card.status==='none' || (card.status==='Pending' && card.Req_type==="withdraw" )) ? 'Processing': (card.status==='Pending' && card.Req_type==="deposit" )? 'Processing':card.status}
                    <br></br>
