...
export default function Homepage({walletUpdate}) {
  ...
  const ChallengeCreate = (e) => {
    if(game_type == "ludo-classic-light-mode" && (Game_Ammount < 50 || Game_Ammount > 25000)){
      Swal.fire({
        title: 'Game amount should be Greater then ₹50 and less then ₹500.',
        icon: "warning",
        confirmButtonText: "OK",
      });
      return;
...
🙏प्लेयर्स कृपया ध्यान दे Popular का code सिर्फ Popular वाले गेम में ही दे अगर आप Classic वाले में Popular का कोड देते है तो आपकी बेट कैंसिल मानी जायगी 🙏
         ...
         */}
          {WebSitesettings?.gameBannerAlert &&<div className="alert alert-danger text-center" role="alert">
          {WebSitesettings?.gameBannerAlert}
        ...
        <p>शर्त राशि: ₹25,000 से ₹1,00,000/-</p>
          ...
          <div className="px-4 py-3">
            ...
            {created &&
             ...
             {allgame && allgame.sort((a, b) => {
                  if (a.Status === "requested" || a.Status === "running") return -1;
                  if (b.Status === "requested" || b.Status === "running") return 1;
                  return 0;
                }) 
                .map((allgame) =>
                  (
                    ...
                    && (allgame.Game_type == game_type || allgame.Game_type == "dummy")
                  ) && (
                    <BetCard
                      key={allgame?._id}
                      allgame={allgame}
