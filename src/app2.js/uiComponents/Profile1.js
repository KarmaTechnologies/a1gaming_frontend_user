  ...
  const [email, setEmail] = useState("");
    ...
    const UserALL = () => {
        ...
        axios.get(baseUrl+`me`, { headers })
            .then((res) => {
//                console.log(res.data.aadhardata[0].name);
                setProfile(res.data)
                setId(res?.data?._id);
...
const validateEmail = (email) => {
  ...
  const handleClick = async () => {
    ...
    if (validateEmail(email) && email !== "") {
        ...
        try {
          const response = await fetch(baseUrl + `changeemail`, {
            method: "POST",
            body: JSON.stringify({
              id: Id,
              email,
          ...
          Swal.fire({
            title: "An error occurred while updating the email.",
            icon: "error",
            confirmButtonText: "OK",
          });
        }
    ...
    return (
        ...
        <Header user={profile} />
            <div className="leftContainer" style={{minHeight:'100vh'}}>
                ...
                <div className="mt-5 py-4 px-3" style={{ background: 'rgb(250, 250, 250)' }}>
                    <div className={`${css.center_xy} py-5`}>
                        ...
                        <br/>
                        <div
              className={`${css.profile_wallet} d-flex align-items-center cursor-pointer`}
              style={{ width: "90%" }}
              onClick={() => {
                setisEmail(true);
              }}
            >
              ...
              {profile?.Email ?<div className={`${css.mytext} ml-5 text-muted`}>{profile?.Email}</div>: <div className={`${css.mytext} ml-5 text-muted`}>Add Email</div>}
  ...
  <input
                ...
                {loading ? <button
                    ...
                    className="w-full flex  justify-center items-center btn btn-danger text-dark uppercase rounded p-2 hover:bg-blue-600"
                  >
                    Wait...
                  </button>: <button
                    onClick={handleClick}
                    className="w-full flex  justify-center items-center btn btn-secondary text-dark uppercase rounded p-2 hover:bg-blue-600"
                  >
                    Add
                  </button>}
                </div>
                        ...
                        <div className={`${css.profile_wallet} d-flex align-items-center mt-3`} style={{ width: '90%' }} onClick={()=>{setiskyc(true)}} >
                            <picture className="ml-4">
                                <img width="32px" src={process.env.PUBLIC_URL + "/Images/kyc-icon-new.png"} alt="" />
                            </picture>
                            <div className={`${css.mytext} ml-5 text-muted`}>
                                View KYC
                            </div>
...
{iskyc && (
  <div>
    <br/>
<center>
  <img src={profile.aadhardata[0].photo_link} alt="profile"/>
    <p>Name: {profile.aadhardata[0].name}</p>
    <p>DOB: {profile.aadhardata[0].dob}</p>
    <p>Address: {profile.aadhardata[0].address}</p>
  </center>
  </div>
)}
                    </div>
                ...
                {/** update Reffer code in profile when not available * 
